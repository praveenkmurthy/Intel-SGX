...
int sqlite3_win32_reset_heap(){
  ...
  winMemAssertMagic();
  if( winMemGetHeap()!=NULL && winMemGetOwned() && sqlite3_int_memory_used()==0 ){
    /*
    ...
    */
    ...
    assert( sqlite3_int_memory_used()==0 );
    ...
    assert( rc!=SQLITE_OK || sqlite3_int_memory_used()==0 );
  }else{
    /*
...
*/
#include <time.h>
struct tm *ocall_localtime(const time_t *t)
{
  static struct tm y;
  FILETIME uTm, lTm;
  SYSTEMTIME pTm;
...
static int winOpen(
  ...
  ** automatically deleted. Nor are they ever temporary files.  */
  ...
  printf("Filename: %s %s %d %p %p \n", zUtf8Name, zName, sizeof(winFile), zName, pFile);
  ...
  */
  if( !zUtf8Name ){
	  printf("Before conversion NULL: %s %s\n", zUtf8Name, zName);
  ...
  ** better if FILE_FLAG_RANDOM_ACCESS is used.  Ticket #2699. */
  ...
  if( osIsNT() ){
	...
	printf("osCreateFileW\n");
#endif
  }
#ifdef SQLITE_WIN32_HAS_ANSI
  else{
	  printf("osCreateFileA");
