    ...
    if( vfsFlags & SQLITE_OPEN_SHAREDCACHE ){
      ...
      if( isMemdb ){
        memcpy(zFullPathname, zFilename, nFilename);
		  ...
		  sqlite3OsFullPathname(&rc, pVfs, zFilename,
                                   nFullPathname, zFullPathname, MAX(nFullPathname,nFilename));
        if( rc ){
  ...
  if( pBt==0 ){
    ...
    */
    ...
    if( pBt==0 ){
	 ...
	 {
	char error_msg[256];
	snprintf(error_msg, 256, "Before Pager Open");
    ...
    if( rc!=SQLITE_OK ){
	 ...
	 {
	char error_msg[256];
	snprintf(error_msg, 256, "After pager open %p %p", pBt, pBt->pPager);
	sqlite3_printf(error_msg);
	}
	...
	{
	char error_msg[256];
	snprintf(error_msg, 256, "After Busy Handler");
	sqlite3_printf(error_msg);
	}
    pBt->pCursor = 0;
    pBt->pPage1 = 0;
    ...
    ** the beginning of the database file. */
	...
	{
	char error_msg[256];
	snprintf(error_msg, 256, "Before Set Page Size");
	sqlite3_printf(error_msg);
	}
   ...
   {
	char error_msg[256];
	snprintf(error_msg, 256, "Before shared cache");
	sqlite3_printf(error_msg);
	}
