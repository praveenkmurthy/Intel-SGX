...
static int vdbeCommit(sqlite3 *db, Vdbe *p){
  ...
  */
    ...
    if( rc==SQLITE_OK ){
      ...
      sqlite3OsOpenMalloc(&rc, pVfs, zMaster, &pMaster,
    ...
    */
    for(i=0; i<db->nDb; i++){
      ...
      if( sqlite3BtreeIsInTrans(pBt) ){
        ...
        sqlite3OsWrite(&rc, pMaster, zFile, sqlite3Strlen30(zFile)+1, offset);
        offset += sqlite3Strlen30(zFile)+1;
        if( rc!=SQLITE_OK ){
          sqlite3OsCloseFree(&rc, pMaster);
          sqlite3OsDelete(&rc, pVfs, zMaster, 0);
          sqlite3DbFree(db, zMaster);
          return rc;
        }
    ...
    */
    if( needSync
     && ( sqlite3OsDeviceCharacteristics(&rc, pMaster) && 0==(rc & SQLITE_IOCAP_SEQUENTIAL))
     && ( sqlite3OsSync(&rc, pMaster, SQLITE_SYNC_NORMAL) && SQLITE_OK!=rc )
    ){
      sqlite3OsCloseFree(&rc, pMaster);
      sqlite3OsDelete(&rc, pVfs, zMaster, 0);
      sqlite3DbFree(db, zMaster);
    ...
    */
    rc = sqlite3OsDelete(&rc, pVfs, zMaster, needSync);
    sqlite3DbFree(db, zMaster);
    zMaster = 0;
    if( rc ){
      return rc;
