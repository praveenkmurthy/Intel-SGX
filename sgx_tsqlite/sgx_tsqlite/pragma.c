...
void sqlite3Pragma(
  ...
  int rc, aFcntlSize;                      /* return value form SQLITE_FCNTL_PRAGMA */
  ...
  */
  ...
  aFcntlSize = (sizeof(aFcntl) + strlen(zLeft) + strlen(zRight));

  db->busyHandler.nBusy = 0;
  rc = sqlite3_file_control(db, zDb, SQLITE_FCNTL_PRAGMA, (void*)aFcntl, aFcntlSize);
  if( rc==SQLITE_OK ){
  ...
  */
  case PragTyp_MMAP_SIZE: {
    ...
    sz = -1;
    rc = sqlite3_file_control(db, zDb, SQLITE_FCNTL_MMAP_SIZE, &sz, sizeof(sz));
#else
    sz = 0;
    rc = SQLITE_OK;
#endif
  ...
  */
  case PragTyp_TEMP_STORE_DIRECTORY: {
    if( !zRight ){
      ...
      if( zRight[0] ){
        int res;
        sqlite3OsAccess(&rc, db->pVfs, zRight, SQLITE_ACCESS_READWRITE, &res);
