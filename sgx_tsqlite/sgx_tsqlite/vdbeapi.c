...
static SQLITE_NOINLINE void invokeProfileCallback(sqlite3 *db, Vdbe *p){
  ...
  assert( p->zSql!=0 );
  sqlite3OsCurrentTimeInt64(&rc, db->pVfs, &iNow);
  ...
  if( p->pc<0 ){
    ...
    */
    ...
    if( db->xProfile && !db->init.busy && p->zSql ){
		int rc;
      sqlite3OsCurrentTimeInt64(&rc, db->pVfs, &p->startTime);
    }else{
      assert( p->startTime==0 );
    }
#endif
...
#endif
  if( *piTime==0 ){
    sqlite3OsCurrentTimeInt64(&rc, p->pOut->db->pVfs, piTime);
    if( rc ) *piTime = 0;
  }
  return *piTime;
}
...
unsigned char *sqlite3_column_text(sqlite3_stmt *pStmt, int i){
  unsigned char *val = (unsigned char *)sqlite3_value_text( columnMem(pStmt,i) );
  columnMallocFailure(pStmt);
  return val;
}
...
#ifndef SQLITE_OMIT_UTF16
void *sqlite3_column_text16(sqlite3_stmt *pStmt, int i){
  void *val = (void*)sqlite3_value_text16( columnMem(pStmt,i) );
  columnMallocFailure(pStmt);
  return val;
}
#endif /* SQLITE_OMIT_UTF16 */
